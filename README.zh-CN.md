...
> ✨ **v0.2.7（即将发布）：** **维基链接**与**反向链接面板**。**Vim 模式**。首次启动**欢迎页**。**GitHub 风格提示框**。**Ctrl+滚轮缩放**。**格式化后保持选中**。**前言编辑器**。预览中渲染图片。**格式工具栏**与**侧边面板切换**。**Nix/NixOS flake 支持**。Linux 门户对话框错误处理。macOS .app 包。20+ 问题修复，包括自动换行滚动、大选区崩溃、IME 退格键等。详见 [CHANGELOG.md](CHANGELOG.md)。
