...
#### Files & session
- **Quick note workflow** (opt-in, **Settings → Files**) — Pathless “scratch” tabs no longer block quit when modified; double-click an untitled tab to set a display name. Unsaved text still persists via session recovery when **Restore previous session on startup** is enabled. Clean exit now keeps `recovery/` content (only the crash snapshot file is cleared) so pathless buffers restore reliably. See `docs/technical/config/quick-note-workflow.md`. *(Post-release: tab close for modified untitled tabs was restored to prompt — see [Unreleased].)*
...
- **CSV rendered view: long cell text overflowed column bounds** — Character-only truncation (50 chars) could still exceed the column pixel width (e.g. long numeric strings), so text spilled into adjacent cells or past the table edge before ellipsis. Cells now clip with `Label::truncate()` at pixel boundaries; tooltips still show full values on hover. See `src/markdown/csv_viewer.rs`, [`docs/technical/viewers/csv-viewer.md`](docs/technical/viewers/csv-viewer.md).
