...
### v0.2.6 (Planned) - Large File Performance & Features

> **Status:** Planned

v0.2.6 focuses on **large file performance** (handling 80MB+ CSV files), code signing, code cleanup, and new features. egui's TextEdit cannot handle massive text buffers efficiently, so large files require a specialized read-only viewing mode.
